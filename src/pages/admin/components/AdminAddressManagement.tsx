  ...
  const fetchAllAddresses = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('addresses')
        .select('*, profiles!addresses_user_id_fkey(full_name, email)')
      ...
      if (error) {
        ...
        if (rawData) {
          // Group addresses by user_id, fetch user info separately
          const userMap = new Map<string, UserAddress>();
          for (const addr of rawData) {
            const uid = addr.user_id;
            if (!userMap.has(uid)) {
              const { data: profile } = await supabase.from('profiles').select('full_name, email').eq('id', uid).single();
              userMap.set(uid, {
                userId: uid,
            ...
            userMap.get(uid)!.addresses.push({
              id: addr.id,
              fullName: addr.full_name || '',
              phoneNumber: addr.phone_number || '',
              email: addr.email || '',
              country: addr.country || '',
              streetAddress1: addr.street_address_1 || addr.address_line1 || '',
              streetAddress2: addr.street_address_2 || addr.address_line2 || '',
              city: addr.city || '',
              state: addr.state || '',
          ...
          setAllAddresses(Array.from(userMap.values()));
        }
      } else if (data) {
        const userMap = new Map<string, UserAddress>();
        for (const addr of data) {
          const uid = addr.user_id;
          const profile = (addr as any).profiles;
          if (!userMap.has(uid)) {
            userMap.set(uid, {
              userId: uid,
              userName: profile?.full_name || 'Unknown User',
        ...
        setAllAddresses(
          allAddresses.map((user) => {
            if (user.userId === selectedUser) {
              return { ...user, addresses: user.addresses.map((a) => (a.id === data.id ? data : a)) };
            }
            return user;
          })
        );
    ...
    if (window.confirm('Are you sure you want to delete this address?')) {
      ...
      if (!error) {
        setAllAddresses(
          allAddresses.map((user) => {
            if (user.userId === selectedUser) {
              return { ...user, addresses: user.addresses.filter((a) => a.id !== addressId) };
            }
            return user;
          })
        );
