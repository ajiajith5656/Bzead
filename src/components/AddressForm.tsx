...
const AddressForm: React.FC<AddressFormProps> = ({
  ...
  const [dbCountries, setDbCountries] = useState<{ short_code: string; country_name: string }[]>([]);

  useEffect(() => {
    const loadCountries = async () => {
      const { data } = await supabase
        .from('countries')
        .select('short_code, country_name')
        .eq('is_active', true)
        .order('country_name');
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          >
            ...
            {dbCountries.map((c) => (
              <option key={c.short_code} value={c.country_name}>{c.country_name}</option>
            ))}
          </select>
